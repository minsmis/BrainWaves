function lfp = callLFP(path)
    ...
    if ismac || isunix
        ...
        % Call lfp
        for f = 1:numel(lfp)
            lfp_a = load(fullfile(lfp(f).folder, lfp(f).name));

            lfp(f).header = lfp_a.header;
            lfp(f).fs = lfp_a.sample_frequency;
            lfp(f).lfp = lfp_a.samples;
            lfp(f).ts = (lfp_a.timestamps - lfp_a.timestamps(1)) / 1000000; % sec
